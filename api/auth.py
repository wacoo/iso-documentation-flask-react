...
def login():
    ...
    # if username in user.__dict__ and users[username] == password:
...
def register():
    ...
    active = data['active']
    try:
        user = User(username = uname, password = pword, first_name=fname, middle_name=mname, last_name=lname, access_level=access_level, active=active)
